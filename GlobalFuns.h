...
/*
 Анализ входных параметров программы:
 -p path [-m [1|2|3] ].
 path - путь к папке с изображениями; является
 возвращаемым из функции значением. Если путь не
 указан, то возвращается 0.
 m - необязательный параметр. Позволяет указать
 режим работы программы(1 - быстрый, 2 - средний, 3 - медленный).
 Чем больше номер режима тем меньше памяти используется программой.
 По умолчанию принимает значение 1. Режим устанавливается внутри
 функции.
*/
